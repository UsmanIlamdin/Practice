-- Run this script directly in the MySQL server query window it will automatically create the
-- database and all the database objects.
...
WHERE pno = pnumber AND lname = 'Smith' AND essn = ssn

UNION

SELECT DISTINCT pnumber
FROM `employee` , `project` , `department`
WHERE dnumber = dnum AND mgrssn =ssn  AND lname = 'Smith'
;

/*
Reteive the name of employee who have
no `dependent`
*/
